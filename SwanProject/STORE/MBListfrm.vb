...
Public Class MBListfrm
    ...
    Function GetData(Optional TOP As Boolean = True, Optional Orderby As String = "") As DataTable
        Try
            Dim strCol As String = mc.SelectbyOrder("MBinput", False)
            With dtACC
                ...
                dt = .QryDT("select " & IIf(TOP, "top " & txtSelectTOP.Text & " ", "") & strCol & " from STOCK  where s_slip like 'MB'  " & ShowEXP & " " & IIf(Orderby = "", "order by s_id desc", Orderby), connstr)
    ...
    Private Sub QAOUT_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        dtt.conn = conn
        dtACC.conn = connacc
        BinddingControl(ColCB, "match_control", "m_fields", "m_fields", connCtrl, " where m_table like 'MBinput' order by m_order")
    ...
    Private Sub GV1_CellContentDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GV1.CellContentDoubleClick
        ...
        frm = New MBfrm
        frm.lblKEY.Text = GV1.Rows(e.RowIndex).Cells("s_code").Value
        frm.txtCode.Text = frm.lblKEY.Text
        frm.Show()
    End Sub
    Sub Edit()
        frm = New MBfrm

        frm.DBCB.Text = DBCB.Text
        frm.lblKEY.Text = GV1.SelectedRows.Item(0).Cells("s_code").Value
        frm.txtCode.Text = frm.lblKEY.Text
        frm.Show()
        frm.txtCode.Focus()
        ' frm.EDITDATA()
    ...
    Private Sub ExcelBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ExcelBtn.Click
        ...
        excelexp.CreateSheet(dv.ToTable, SaveFileDialog1.FileName, "MB")
    End Sub
